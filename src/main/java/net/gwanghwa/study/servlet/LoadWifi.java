        ...
        try {
        	...
        	if(response.isSuccessful()) {
        		...
        		if(jsonElement.isJsonObject()) {
	        	    ...
	        	    int nListTotCnt = Integer.parseInt(jsonObject.get("list_total_count").toString());
					...
					if ("INFO-000".compareTo(apiRsultList.getCode()) == 0) {
						int arrayCnt = (int) Math.ceil(nListTotCnt / (double) unitCnt);
						for (int i = 0; i < arrayCnt; i++) {
							nStaIdx = (unitCnt * i) + 1;
							nEndIdx = nStaIdx + unitCnt - 1;
							if (nEndIdx > nListTotCnt)
								nEndIdx = nListTotCnt;

							System.out.println("[LoadWifi::getSeoulPublicWifiApi] " + requestUrl + nStaIdx + "/" + nEndIdx);
							...
							try {
								...
								if (response.isSuccessful()) {
									...
									if (jsonElement.isJsonObject()) {
										...
										commonDBMSService.setAutoCommit(false);
										...
										commonDBMSService.setAutoCommit(true);
						...
						System.out.println("API 통신 오류 [" + apiRsultList.getCode() + "] " + apiRsultList.getMessage());
					}
        		}
